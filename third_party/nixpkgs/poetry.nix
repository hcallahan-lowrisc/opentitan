{ self, ... }: {
  ...
  imports = [
    # Declares the perSystem output 'poetryPackagesList'
    ./poetryPackagesList.nix
  ];
  ...
  in
    {
      ...
      # Custom output attribute 'poetryPackages' declared in './poetryPackages.nix'
      # Used specifically so the list of packages can be a flake output, as
      # rules_nixpkgs:nixpkgs_python_repository needs to consume them like so. 
      poetryPackagesList = poetryPackages.poetryPackages;
    };
}
