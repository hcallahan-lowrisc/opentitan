...
class hmac_base_vseq extends cip_base_vseq #(.CFG_T               (hmac_env_cfg),
    ...
    fork : sar_simple_thread
      ...
      begin
        ...
        while (msg_q.size() > 0) begin
          ...
          if (bits_written % get_block_size(digest_size) == 0) begin
            ...
            if (sar_window.get_num_waiters() > 0) begin
              sar_window.trigger();
              // When message is not completely yet transmitted, wait until HASH Continue is raised
              if (msg_q.size() > 0) hash_continue.wait_trigger();
            end
          end
  ...
  // TODO: remove this check from the seq (issue #23288)
