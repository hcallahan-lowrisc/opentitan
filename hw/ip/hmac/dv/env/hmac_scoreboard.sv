        ...
        case (csr_name)
          "cmd": begin
            ...
            if (item.a_data[HashStop]) begin
              ...
              mult  = (hmac_rd_cnt * 32) / block_size;
              mod   = (hmac_rd_cnt * 32) % block_size;
              msg_slice_length = mod > 0 ? (mult + 1) * block_size / 8 : mult * block_size / 8;

              // In case the remaining message is less than a block size, push only this remaining
              // message into the queue. Divide by 8 as message queue is taking data in bytes.
              msg_slice_length = msg_slice_length > msg_q.size() ? msg_q.size() : msg_slice_length;
          ...
          "wipe_secret": begin
            // TODO (#): when writing wipe_secret, trigger RTL assertion checks
          end
    ...
    if (!write) begin
      case (csr_name)
        ...
        "digest_14", "digest_15": begin
          ...
          // TODO later, modify the C model to be able to provide intermediate digest (issue #23240)
