...
static size_t spi_device_send_frame(ottf_console_t *console, const char *buf,
  ...
  // If using the GPIO Tx-ready indicator pin, toggle it high to signal to the host
  // there is data in the egress buffer, and then block until the host reads out the
  // entire frame. Clear the Tx-ready indicator pin before continuing.
  if (console->data.spi.tx_ready_gpio != kOttfSpiNoTxGpio) {
    ...
    // header, followed by the N-byte payload. Each transfer is framed on the wire by
    // observing the chip-select first toggling high->low then low->high.
    //
    // After the first toggle low, when the host begins reading out the frame header,
    // we can deassert the TX-ready pin as the host will continue to complete two SPI
    ...
    for (size_t i = 0; i < 4; ++i) {
      ...
      // After the first toggle of CSB (high->low, the start of the header transfer),
      // the HOST is already going to queue up and complete 2 full transfers, so we can
      // already de-assert the Tx-Ready indicator GPIO.
      if (i == 0) {
        OT_DISCARD(dif_gpio_write(&ottf_console_gpio,
